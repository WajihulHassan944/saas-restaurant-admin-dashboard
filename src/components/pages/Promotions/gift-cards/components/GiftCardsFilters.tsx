...
export type GiftCardsFilterState = {
  search: string;
  lifecycle: string;
  branchId: string;
};

export type GiftCardBranchOption = {
  id: string;
  name: string;
};

type GiftCardsFiltersProps = {
  filters: GiftCardsFilterState;
  branchOptions: GiftCardBranchOption[];
  isBranchAdmin: boolean;
  branchName?: string;
  visibleCount: number;
  totalCount: number;
  isFetching: boolean;
  onFiltersChange: (filters: Partial<GiftCardsFilterState>) => void;
};

export default function GiftCardsFilters({
  filters,
  branchOptions,
  isBranchAdmin,
  branchName,
    ...
    <div className="w-full max-w-full overflow-hidden rounded-[20px] border border-gray-100 bg-white p-4 shadow-sm">
      <div className="mb-4 flex min-w-0 flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-gray-500">{t("filtersDescription")}</p>
          </div>
        </div>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-12 xl:items-end">
        ...
        <Button
          ...
          onClick={handleReset}
          className="h-[44px] rounded-[14px] border-gray-200 px-4 text-gray-700 md:w-full xl:col-span-2"
        >
